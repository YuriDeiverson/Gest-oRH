...
const MemberDashboardNew: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const membersResponse = await fetch(
        "http://localhost:3001/api/members/public/list",
      );
      ...
      const myIndicationsResponse = await fetch(
        `http://localhost:3001/api/intentions/public/list?referredBy=${memberId}`,
      );
  ...
  const handleIndicationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/referrals/refer/${memberId}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const updateReferralStatus = async (
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/intentions/${referralId}/tracking-status`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
